...
static int bq2560x_set_charger_type_desc(struct bq2560x *bq)
{
	int rc = 0;
	union power_supply_propval prop = {0, };

	/* Read power supply type prop from usb psy */
	...
	/* Set usb power_supply_desc type */
...
static void bq2560x_external_power_changed(struct power_supply *psy)
{
	...
	ret = bq2560x_set_charger_type_desc(bq);
	if (ret < 0) {
		pr_info("could not set charge type, ret=%d\n", ret);
	}
...
static irqreturn_t bq2560x_charger_interrupt(int irq, void *dev_id)
{
	...
	if(!bq->power_good) {
		...
		/* Set power supply type to USB when USB is removed */
		bq->usb_psy_desc.type = POWER_SUPPLY_TYPE_USB;
